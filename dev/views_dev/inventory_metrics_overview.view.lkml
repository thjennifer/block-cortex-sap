view: inventory_metrics_overview {
  ...
  dimension: pk {
    ...
    primary_key: yes
    hidden: yes
    sql: concat(${client_mandt},${language_spras},${material_number_matnr},${month_end_date_pk},${plant_werks}) ;;
  # Client_MANDT||LanguageKey_SPRAS||MaterialNumber_MATNR||MonthEndDate||Plant_WERKS
  # maybe include CompanyCode_BUKRS (no impact on demo data)
  }
  ...
  measure: sum_inventory_value_target_currency {
    ...
    link: {
      ...
      url: "/dashboards/cortex_sap_operational_prod::stock_value_details?Company+Name={{ _filters['inventory_metrics_overview.company_text_butxt']| url_encode }}&Currency={{ _filters['inventory_metrics_overview.target_currency']| url_encode }}&Plant={{ _filters['inventory_metrics_overview.plant_name2_name2']| url_encode }}&Material={{ _filters['inventory_metrics_overview.material_text_maktx']| url_encode }}&Country={{ _filters['inventory_metrics_overview.country_key_land1']| url_encode }}&Stock+Type={{ _filters['inventory_by_plant.stock_characteristic']| url_encode }}&Material+Type={{ _filters['inventory_metrics_overview.material_type']| url_encode }}"
    }
  }
